...
# result is not unique because any particular number is a reversed form 
# of several numbers (e.g. 21 could be 12, 120 or 1200 before reversing). 
...
def reverseNum(number):
	rev = reversed(str(number))
	revNum = int(''.join(rev))
	return revNum

for i in range(n):
	a, b = input().split()
	revSum = reverseNum(a)+reverseNum(b)
	print(reverseNum(revSum))
