...
#  
# ACM technicians faced a very interesting problem recently. Given a set 
# of BTSes to visit, they needed to find the shortest path to visit all 
# of the given points and return back to the central company building. 
# Programmers have spent several months studying this problem but with no 
# results. They were unable to find the solution fast enough. After a long 
# time, one of the programmers found this problem in a conference article. 
# Unfortunately, he found that the problem is so called "Travelling Salesman 
# Problem" and it is very hard to solve. If we have N BTSes to be visited, 
# we can visit them in any order, giving us N! possibilities to examine. 
# The function expressing that number is called factorial and can be computed 
# as a product 1.2.3.4....N. The number is very high even for a relatively 
# small N.
# 
...
# is the number of zeros at the end of the decimal form of number N!. They noticed 
# that this function never decreases. If we have two numbers N1<N2, then 
# Z(N1) <= Z(N2). It is because we can never "lose" any trailing zero by multiplying 
# by any positive number. We can only get new and new zeros. The function Z is very 
